...
@TestInstance(TestInstance.Lifecycle.PER_CLASS) // @BeforeAll and @AfterAll methods can be non-static
public abstract class BaseIntegrationSuite {
    ...
    static {
        POSTGRES.start(); //this will also register a JVM shutdown hook that calls POSTGRES.stop() which ensures container is cleanly stopped when JVM stops
    }
}
